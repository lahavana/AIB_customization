 ...
 $osOptURL = 'https://sawvdautimation.blob.core.windows.net/wvdoptimization/Windows_10_VDI_Optimize-master.zip'
 ...
 $updatePath= ".\Win10_VirtualDesktop_Optimize.ps1"
 ...
 #add gc and sleep
 $insert | ForEach-Object { $file.insert($_,"Write-Host 'Patch closing handles and runnng GC before reg unload' `
 $newKey.Handle.close() `
    [gc]::collect() `n                
...
'All','WindowsMediaPlayer','AppxPackages','ScheduledTasks','DefaultUserSettings','Autologgers','Services','NetworkOptimizations','LGPO','DiskCleanup'
Default = "All",
#>
 # run script
...
#reboot the VM
read-host -Prompt "Press any key to reboot the VM"
Restart-Computer
